...
def make_shirt(size, message):
    ...
    print(f"the message should be: {message}\n")


make_shirt(38, 'vegan')
make_shirt(size=42, message='veganuary')


# Large Shirts: Modify the make_shirt() function so that shirts are large by default with a message
# that reads I love Python. Make a large shirt and
# a medium shirt with the default message, and a shirt of any size with a different message.

def make_shirt(size='xxxl', message='I love Python'):
    """summarize shirt going to made"""
    print(f"the size of shirt: {size}")
    print(f"the message should be: {message}\n")
...
print(city)
city = city_country('auckland', 'new-zealand')
print(city)
...
album = make_album('metallica', 'ride the lightning')
print(album)
album = make_album('beethoven', 'ninth symphony', tracks=7)
print(album)
album = make_album('willie nelson', 'red-headed stranger', 9)
print(album)


# User Albums: Start with your program from above. Write a while loop that allows users to
...
messages = ['hello', 'how are you?', 'fine']
show_messages(messages)


# Sending Messages: Start with a copy of your program from above
# Write a function called send_messages() that prints each text message and
# moves each message to a new list called sent_messages as it’s printed. After calling the function,
...
# Archived Messages: Start with your work from Exercise 8-10. Call the function send_messages() with a copy
# of the list of messages. After calling the function, print both of your lists to show that the
...
make_sandwich('tofu', 'vegan cheese', 'lettuce')
make_sandwich('olives', 'apple slices', 'vegan mustard')
make_sandwich('peanut butter', 'strawberry jam')


# User Profile: Start with a copy of user_profile.py from page 149. Build a profile of yourself by
# calling build_profile(), using your first and last names and three other key-value pairs that describe you.

def build_profile(first, last, **profiles):
    user = {
        'first_name': first,
        'last_name': last,
    }
...
user_profile = build_profile('divi', 'jesh', location='hyd', field='developer', hobby='surfing')
print(user_profile)


# Cars: Write a function that stores information about a car in a dictionary.
# The function should always receive a manufacturer and a model name. It should then accept an arbitrary number of
